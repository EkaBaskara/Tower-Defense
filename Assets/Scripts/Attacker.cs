...
public class Attacker : MonoBehaviour
{
        ...
        if (isPlayer)
        {
            ...
            else
            {
                ...
                if (timer > 0.6f)
                {
                    ...
                    Debug.Log(gameObject.name + "Take Damage (isPlayer)");
        ...
        else
        {
            ...
            else
            {
                ...
                if (timer > 0.6f)
                {
                    ...
                    Debug.Log(gameObject.name + "Take Damage (isEnemy)");
                    transform.localScale = new Vector2(1, 1f);
                    timer = 0;
                }
                else if (timer > 0.5f)
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        if (collision.transform.tag.Equals(nameTagLawan) && isMove)
        {
            ...
            if (m != null)
            {
                Debug.Log(collision.gameObject.name + "Under Attack");
